...
- (NSArray *)telecomsAddresses {
    ...
    NSMutableArray *telsM = [NSMutableArray arrayWithArray:tels];
    NSMutableArray *telsM1 = [NSMutableArray array];
    
    for (NSXMLElement *ele in telsM) {
        
        for (NSXMLElement *childEle in [ele children]) {
            
            if ([childEle.name isEqualToString:@"WORK"]) {
                [telsM1 addObject:ele];
                break;
            }
        }
    ...
    NSXMLElement *telEle = nil;
    for (NSXMLElement *ele in telsM1) {
        
        for (NSXMLElement *childEle in [ele children]) {
            
            if ([childEle.name isEqualToString:@"CELL"]) {
    ...
    NSXMLElement *numEle = [telEle elementForName:@"NUMBER"];
    return @[[numEle stringValue]];
...
- (void)setEmailAddresses:(NSArray *)emails {
    ...
    NSXMLElement *userIdEle = [NSXMLElement elementWithName:@"USERID" stringValue:emailStr];
